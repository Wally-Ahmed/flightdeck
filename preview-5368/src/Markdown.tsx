...
const MARKDOWN_SANITIZE_SCHEMA = {
  ...
  // ── PREVIEW-ONLY DEVIATION (not in the upstream #5368 file) ──
  // rehype-sanitize's defaultSchema only allows http/https/irc/ircs/mailto/xmpp
  // for `a.href`, so `node:`/`run:` hrefs are stripped before MarkdownLink runs
  // — which means the #5368 mention-chip code paths never fire on their own.
  // We whitelist those two in-app protocols here so this standalone preview can
  // actually demonstrate the chips. (Likely a real gap in the PR: the upstream
  // app must allow these protocols somewhere for the feature to work.)
  protocols: {
    ...(defaultSchema.protocols ?? {}),
    href: [...(defaultSchema.protocols?.href ?? []), "node", "run"],
  },
};
